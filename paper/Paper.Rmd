...
us_map %>% 
  ...
  geom_polygon(aes(x=long, 
                   ...
                   group = group)) + 
  coord_fixed(1.3) +
  theme_tufte(base_size = 20, base_family = "serif") +
...
```{r, echo = FALSE, fig.height=1, fig.cap='Median Adjusted Income per person (left) and per bedroom (right)'}

ggplot() + theme(panel.background = element_rect(fill = 'white', colour = 'white'))
```

```{r, echo = FALSE, fig.width=3.5, fig.height=5, fig.cap='one plotthe other one'}
clean_years %>%
...
Figure 3: Median Adjusted Income per person (left) and per bedroom (right)
  ...
  The second plot of figure 3 depicts the development of median income adjusted for the number of bedrooms in a household's living quarters. The adjusted median income was calculated using precaculated HUD indexes. Here, zero bedrooms was adjusted by a factor of 0.70, two bedrooms by 0.90, and so on. Three bedroom properties were adjusted by a factor of 1.04, because the median number of bedrooms in a property is roughly three bedrooms
...
```
  ...
  As pictured in figure 5, the relationship between household income and burden is seen as a positive correlation. From the graph, you can see that higher income household have less burden to deal with, which makes sense because those with more money are able to pay off monthly costs more easily. Those with 50% more burden are near the poverty line. Figure 5 also shows a spike in households that have burden. Households with higher incomes started to pay more burdens, which can be attributed to 2007 financial housing market crisis.
...
```
  ...
  From figure 7 you can see that the Northeast and West have higher rates of burden at higher income levels, therefore illustrating how housing affordability in these regions are less than that of the Midwest and South.
